...
if plt
    ...
    %... add title
    tx1 = sprintf('Coronavirus epidemic in %s on: %s (SIR model)',country,datetime(last_day,'format','MM-dd-yy'));
    tx2 = sprintf('%s %g  %s %g  %s %g  %s %g  %s %g  %s %g  %s %g',...
            '\sigma = ',round(b(1)/b(2),2),...
            'T_c = ',round(1/b(1),2),...
            'T_r = ',round(1/b(2),2),...
            'N = ',round(b(3),0),...
            'C_{end} = ',round(Clim,0),...
    ...
    datetick('x',2,'keeplimits')
    %... add title
    tx1 = sprintf('Infection Rate in: %s on: %s',country,datetime(last_day,'format','MM-dd-yy'));
